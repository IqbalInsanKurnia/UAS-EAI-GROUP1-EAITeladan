...
app.post('/add', async (req, res) => {
  try {
    // Mengambil ID yang disertakan dalam body permintaan
    const { paket_id } = req.body;

    // Memastikan ID disertakan dalam body permintaan dan memiliki nilai yang valid
    ...
    // Mengembalikan respons dengan sukses dan ID dokumen yang baru ditambahkan
    res.status(200).json({ success: true, paket_id });
  } catch (error) {
    // Mengembalikan respons dengan pesan kesalahan jika terjadi error
